 ...
 */
/**
 * useBackendThread â€” Fetches thread from backend with Dilithium5 verification.
    ...
    async function fetchAndVerify() {
      try {
        ...
        if (wasmReady && data.signature && data.pubkey) {
          ...
          if (!cancelled) {
            if (result === 0) {
              console.log(`[PQC] âœ… Thread #${threadId} signature verified (Dilithium5)`);
              setVerified(true);
            } else if (result === null) {
              setVerified(null);
            } else {
              console.error(`[PQC] âŒ Thread #${threadId} SIGNATURE INVALID`);
              setThread(null);
              setVerified(false);
            }
          }
